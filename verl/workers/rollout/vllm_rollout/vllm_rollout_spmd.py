...
class vLLMRollout(BaseRollout):  # we got this rollout
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if is_validate:
            # check if multi modal is empty
            for inp in vllm_inputs:
                if not inp.get("multi_modal_data"):
                    inp.pop("multi_modal_data", None)
        ...
        with self.update_sampling_params(**kwargs):
            ...
            response = []
            for out_id, output in enumerate(outputs): # for each sample
                for sample_id in range(len(output.outputs)): # repeat n times
